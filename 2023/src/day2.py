...
def parse_row(row: str) -> Tuple[bool, int]:
    ...
    possible = all(
        all(color_limit.get(bag.split()[1], 0) >= int(bag.split()[0]) for bag in bags)
...
@lru_cache(2)
def parse_row_two(row: str) -> int:
    game, cubes = row.split(":")
    _, _game_id = game.split()
    cubes = cubes.split(";")
    bags = (cube.strip().split(", ") for cube in cubes)
    bags = [{color.split()[1]: int(color.split()[0]) for color in bag} for bag in bags]
    min_cube = [max(bag.get(color, 0) for bag in bags) for color in color_limit.keys()]
